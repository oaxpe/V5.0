...
CREATE TABLE IF NOT EXISTS `dendaProiektua`.`salmenta` (
  ...
  `salmData` VARCHAR(10) NULL,
  `salmPrezioTotala` DOUBLE NULL,
  `salmProdKantitatea` INT NULL,
  `produktua_prodId` INT NOT NULL,
  PRIMARY KEY (`salmKode`, `produktua_prodId`),
  INDEX `fk_salmenta_produktua1_idx` (`produktua_prodId` ASC),
  CONSTRAINT `fk_salmenta_produktua1`
    FOREIGN KEY (`produktua_prodId`)
    REFERENCES `dendaProiektua`.`produktua` (`prodId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
